...
namespace LinkTranslator
{
    public class HtmlTag
    {
        ...
        /// <returns>True if tag contains attribute or false otherwise. IF the requested
        /// tag has been found the scanner position is at the first character behind the
        /// closing '>' of the tag.</returns>
    ...
    public class HtmlParser : TextScanner
    {
        public HtmlParser()
        {
        }
        ...
        public bool ParseNext(string name, out HtmlTag tag)
        {
            ...
            while (!EndOfText)
            {
                ...
                // Find next tag and store tag text in current tag
                MoveTo('<');
            }
        ...
        public string ParseInnerText(HtmlTag tag, bool reduceWhiteSpace = true)
        {
            ...
            while (!EndOfText)
            {
                // collect the text to the next tag start
                int start = Position;
                MoveTo('<');
                sb.Append(Substring(start, Position));
                ...
                if (ParseTag(endTagName, ref tempTag, out inScript))
                    break;
            }
            string fullTagText = sb.ToString();
            if (reduceWhiteSpace)
                ReduceWhitespace(ref fullTagText);
            return fullTagText;
        }
